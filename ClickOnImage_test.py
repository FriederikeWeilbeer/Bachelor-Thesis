...
def click_event(event, x, y, flags, params):
    ...
    # left-click on the image to choose a point to which the robot will drive
    if event == cv2.EVENT_LBUTTONDOWN:
        cooX = x
        cooY = y
        print(str(x) + " " + str(y))
        socket.send_string("14 go " + str(x) + " " + str(y))

    # right click to send cal, used for calibration and other test things
    if event == cv2.EVENT_RBUTTONDOWN:
        socket.send_string("14 cal")
...
def main():
    ...
    while True:
        ...
        # draw point at chosen coordinates
        if cooX and cooY != None:
            font = cv2.FONT_HERSHEY_SIMPLEX
            cv2.putText(img, str(cooX) + ',' +
                        str(cooY), (cooX, cooY), font,
                        1, (255, 0, 0), 2)
